...
class VehicleDetectionMenu(QMainWindow):
    def __init__(self):
        # 调用父类QMainWindow的初始化方法
        super(VehicleDetectionMenu, self).__init__()
        # 设置窗口标题
    ...
    def enter_live_monitor(self):
        ...
        try:
            ...
            self.live_monitor_menu = LiveMonitorMenu()  # 创建LiveMonitorMenu实例
    ...
    def select_video_file(self):
        ...
        try:
            ...
            fileName, _ = QFileDialog.getOpenFileName(self, "选择视频文件", "", "Video Files (*.mp4 *.avi);;All Files (*)",
                                                      options=options)
            if fileName:
                from menu.video_analysis_menu import VideoAnalysisMenu  # 延迟导入VideoAnalysisMenu模块
                self.video_analysis_menu = VideoAnalysisMenu(fileName)  # 创建VideoAnalysisMenu实例，传入选择的视频文件
